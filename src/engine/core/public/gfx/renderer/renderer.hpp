...
class RenderPassInstanceBase
{
  public:
    ...
    [[nodiscard]] const std::weak_ptr<RenderPassObject>& get_render_pass() const
    {
        return render_pass;
    }
...
{
  public:
    SwapchainPresentPass(std::shared_ptr<RenderPassObject> render_pass, std::weak_ptr<Swapchain> target, std::shared_ptr<RendererStep> present_step);
