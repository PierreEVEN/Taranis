...
namespace Engine
{
	...
	public:
		...
		void declare_render_pass(const std::string& render_pass, const RenderPassInfos& render_pass_infos);

		std::shared_ptr<RenderPass> get_render_pass(const std::string& render_pass);


	private:

		std::unordered_map<std::string, RenderPassInfos> render_passes_declarations;
		std::unordered_map<std::string, std::shared_ptr<RenderPass>> render_passes;
