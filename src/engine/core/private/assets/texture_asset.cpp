...
TextureAsset::TextureAsset(const std::vector<Gfx::BufferData>& mips, const CreateInfos& create_infos)
    : width(create_infos.width), height(create_infos.height), depth(create_infos.depth), format(create_infos.format), generate_mips(create_infos.generate_mips)
{
    switch (create_infos.format)
    {
    ...
    case Gfx::ColorFormat::R8G8B8_UNORM:
    {
        ...
        image = Gfx::Image::create(get_name(), Engine::get().get_device(), Gfx::ImageParameter{
                                       ...
                                       .array_size = create_infos.array_size
                                   },
                                   rgba_mips);
        break;
    }
