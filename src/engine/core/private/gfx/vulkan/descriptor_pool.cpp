...
namespace Engine
{
	...
	PoolDescription::PoolDescription(const Pipeline& pipeline)
	{
		...
		std::ranges::sort(pool_sizes, [](const VkDescriptorPoolSize& a, const VkDescriptorPoolSize& b)
		{
	...
	DescriptorPool::Pool::Pool(std::weak_ptr<Device> in_device, const PoolDescription& description) : initial_space(20),
	                                                                                                  device(std::move(in_device))
	{
		...
		const VkDescriptorPoolCreateInfo poolInfo{
			...
			.maxSets = space_left,
			.poolSizeCount = static_cast<uint32_t>(description.pool_sizes.size()),
			.pPoolSizes = description.pool_sizes.data(),
		};

		VK_CHECK(vkCreateDescriptorPool(device.lock()->raw(), &poolInfo, nullptr, &ptr),
	...
	VkDescriptorSet DescriptorPool::Pool::allocate(const VkDescriptorSetLayout& layout)
	{
		...
		VK_CHECK(vkAllocateDescriptorSets(device.lock()->raw(), &descriptor_info, &descriptor_set),
