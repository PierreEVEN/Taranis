...
namespace Engine
{
RenderPassObject::RenderPassObject(const std::shared_ptr<Device>& in_device, RenderPassInfos in_infos) : infos(std::move(in_infos)), device(in_device)
{
    ...
    VK_CHECK(vkCreateRenderPass(in_device->raw(), &renderPassInfo, nullptr, &ptr), "Failed to create render pass")
}
...
RenderPassInstanceBase::RenderPassInstanceBase(std::shared_ptr<RenderPassObject> in_render_pass, std::shared_ptr<RenderPassInterface> in_interface)
    : render_pass(in_render_pass), device(in_render_pass->get_device()), interface(in_interface)
{
...
SwapchainPresentPass::SwapchainPresentPass(std::shared_ptr<RenderPassObject> render_pass, std::weak_ptr<Swapchain> in_target, std::shared_ptr<RendererStep> present_step)
    : RenderPassRoot(render_pass, present_step), swapchain(in_target)
{
...
void SwapchainPresentPass::resize(glm::uvec2 parent_resolution)
{
    ...
    for (size_t i = 0; i < image_count; ++i)
        framebuffers.emplace_back(std::make_shared<Framebuffer>(render_pass.lock()->get_device(), *this, image_index++));
}
...
InternalPassInstance::InternalPassInstance(std::shared_ptr<RenderPassObject> render_pass, std::shared_ptr<RenderPassInterface> interface) : RenderPassInstanceBase(render_pass, interface)
{
}
...
RenderPassRoot::RenderPassRoot(std::shared_ptr<RenderPassObject> render_pass_object, std::shared_ptr<RendererStep> present_pass) : RenderPassInstanceBase(render_pass_object, present_pass->get_interface())
{
    ...
    while (!remaining.empty())
    {
        ...
        remaining.pop_back();
        instanced_passes.emplace(def, std::make_shared<InternalPassInstance>(render_pass_object->get_device().lock()->find_or_create_render_pass(def->get_infos()), def->get_interface()));
