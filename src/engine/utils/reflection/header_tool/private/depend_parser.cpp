...
DependParser::DependParser(const std::filesystem::path &path) {
    if (auto error = parse(path)) {
        std::cout << path << ":" << error->location.line << ":" << error->location.column << " : " <<
                "Failed to parse depend file : " << error->message << "\n";
        exit(EXIT_FAILURE);
    }
}
...
namespace Llp {
    ...
    DECLARE_LEXER_TOKEN(MsvcDependLiteralToken)
        ...
        static std::unique_ptr<MsvcDependLiteralToken> consume(Llp::Lexer &, Llp::Location &in_location,
...
std::optional<Llp::ParserError> DependParser::parse(const std::filesystem::path &path) {
    ...
    source_header->read();
    Llp::Lexer lexer;
    lexer.register_token_before<Llp::MsvcDependLiteralToken, Llp::StringLiteralToken>("MsvcLiteralString");

    lexer.run(source_header->raw_stream());
    if (auto lexer_error = lexer.get_error()) {
        std::cerr << path << ":" << lexer_error->location.line << ":" << lexer_error->location.column << " : " <<
                lexer_error->message << "\n";
        exit(EXIT_FAILURE);
    }
    ...
    if (auto main_block = main_parser.consume<Llp::BlockToken>()) {
            ...
            if (key->word == "files") {
                if (auto dep_list_block = key_list_block.consume<Llp::BlockToken>()) {
                    ...
                    do {
                        ...
                        else if (auto dep_path_msvc = dependencies_block_parser.consume<Llp::MsvcDependLiteralToken>())
                            dependencies.emplace_back(dep_path_msvc->value);
                        else
                            return Llp::ParserError{
                                dependencies_block_parser.current_location(), "expected string literal : \"\""
                            };
                    ...
                    if (dependencies_block_parser.get_current_token_type() != Llp::NULL_TOKEN) {
                        return Llp::ParserError{dependencies_block_parser.current_location(),
                            std::format("expected end of block, got {}",
                                        lexer.get_context().get_token_name(dependencies_block_parser.get_current_token_type()))
                        };
                    }
                    return {};
                }
                return Llp::ParserError{
                    key_list_block.current_location(),
                    std::format("expected file block, got {}",
                                lexer.get_context().get_token_name(key_list_block.get_current_token_type()))
                };
            }
        ...
        return Llp::ParserError{
            main_parser.current_location(),
            std::format("expected main block, got {}",
                        lexer.get_context().get_token_name(main_parser.get_current_token_type()))
        };
    }
    return {};
}
