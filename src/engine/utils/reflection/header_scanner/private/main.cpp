...
public:
    std::time_t get_time_for(const std::filesystem::path &file_path, bool is_dep_file) { {
            std::shared_lock lk(cache_lock);
            if (auto it = time_cache.find(file_path); it != time_cache.end())
                return it->second;
        }

        std::unique_lock lk(cache_lock);
    ...
    static bool parse_dep_file(const std::filesystem::path &file_path, std::vector<std::filesystem::path> &deps) {
        ...
        while (f.get(c)) {
            ...
            switch (step) {
                ...
                case 1: {
                    if (c == 'f' && f.get(c) && c == 'i' && f.get(c) && c == 'l' && f.get(c) && c == 'e' && f.get(c) &&
                        c == 's')
                        step++;
                    else
                        return true;
...
private:
    static bool expect(std::vector<std::string>::const_iterator &it,
...
int main(int argc, char **argv) {
    ...
    DepCache dep_cache;
    JobSystem js;
    std::vector<JobHandle<void> > tasks;

    // Generate .gen.hpp first (required to compile .gen.cpp then)
    for (const auto &file: opts.files) {
        js.schedule([&file, &dep_cache, &opts] {
            ...
            if (const auto code =
                    std::system(std::format("{} {} {} {} {}", opts.header_tool_path.string(), file.header_path.string(),
