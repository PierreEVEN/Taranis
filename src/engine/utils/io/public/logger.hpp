...
public:
    ...
    struct LogItem {
        LogItem(LogType in_log_level, const std::string &in_message, const char *in_function = nullptr,
                size_t in_line = 0, const char *in_file = nullptr)
    ...
    void set_log_file(std::format_string<Args...> fmt) {
        time_t now = time(0);
        struct tm tstruct;
        char buf[80];
#if OS_WINDOWS
        ...
        auto formated_file_path = std::format(fmt.c_str(), buf);

        const auto log_folder = std::filesystem::path(formated_file_path).parent_path();
        ...
        if (log_file && *log_file)
            log_file->close();
        log_file = std::make_unique<std::ofstream>(formated_file_path);
    }
