...
namespace Eng::Gfx {
    ...
    Buffer::Buffer(std::string in_name, std::weak_ptr<Device> in_device, const CreateInfos &create_infos,
                   size_t in_stride, size_t in_element_count)
        ...
        switch (params.type) {
            ...
            case EBufferType::IMMEDIATE:
                for (size_t i = 0; i < device.lock()->get_image_count(); ++i)
                    buffers.emplace_back(std::make_shared<Resource>(name + "_#" + std::to_string(i), device,
                                                                    create_infos, in_stride, in_element_count));
                break;
        }
    }
            ...
            case EBufferType::DYNAMIC:
                ...
                for (size_t i = 0; i < device.lock()->get_image_count(); ++i)
                    buffers.emplace_back(
                        std::make_shared<Resource>(name, device, params, new_stride, new_element_count));
                break;
            case EBufferType::IMMEDIATE:
                auto &current_buffer = buffers[device.lock()->get_current_image()];
                device.lock()->drop_resource(current_buffer);
                buffers[device.lock()->get_current_image()] = std::make_shared<Resource>(
                    name, device, params, new_stride, new_element_count);
        }
        return true;
    }
    ...
    Buffer::Resource::Resource(const std::string &in_name, std::weak_ptr<Device> in_device,
                               const CreateInfos &create_infos, size_t in_stride, size_t in_element_count)
        : DeviceResource(in_name, std::move(in_device)), stride(in_stride), element_count(in_element_count) {
        ...
        VK_CHECK(
            vmaCreateBuffer(device().lock()->get_allocator().allocator, &buffer_create_info, &allocInfo, &ptr, &
                vma_alloc, nullptr), "failed to create buffer")
    ...
    void Buffer::Resource::set_data(size_t start_index, const BufferData &data) {
        if (host_visible) {
            ...
            VK_CHECK(vmaMapMemory(device().lock()->get_allocator().allocator, allocation->allocation, &dst_ptr),
            ...
            transfer_buffer = create("transfer_buffer", device(), CreateInfos{.usage = EBufferUsage::TRANSFER_MEMORY},
                                     data);
