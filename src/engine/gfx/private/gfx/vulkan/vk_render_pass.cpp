...
VkRendererPass::VkRendererPass(std::string in_name, const std::weak_ptr<Device>& in_device, RenderPassKey in_key) : name(std::move(in_name)), key(std::move(in_key)), device(in_device)
{
