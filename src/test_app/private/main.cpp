...
int main()
{
    ...
    main_window.lock()->set_renderer(
        Engine::Renderer::create<TestFirstPassInterface>("present_pass", {.clear_color = Engine::ClearValue::color({0.2, 0.2, 0.5, 1})}, main_window)
            ->attach(Engine::RendererStep::create("forward_pass", {Engine::Attachment::color("color", Engine::ColorFormat::R8G8B8A8_UNORM), Engine::Attachment::depth("depth", Engine::ColorFormat::D24_UNORM_S8_UINT)}))
            ->attach(Engine::RendererStep::create("forward_test", {Engine::Attachment::color("color", Engine::ColorFormat::R8G8B8A8_UNORM), Engine::Attachment::color("normal", Engine::ColorFormat::R8G8B8A8_UNORM),
    ...
    secondary_window.lock()->set_renderer(
        Engine::Renderer::create<TestFirstPassInterface>("present_pass", {.clear_color = Engine::ClearValue::color({0.2, 0.2, 0.5, 1})}, secondary_window)
            ->attach(Engine::RendererStep::create<TestFirstPassInterface>("forward_pass",
                                                                          {Engine::Attachment::color("color", Engine::ColorFormat::R8G8B8A8_UNORM, Engine::ClearValue::color({1, 0.2, 0.5, 1})),
