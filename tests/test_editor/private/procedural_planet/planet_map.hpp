...
{
    const double inv_sqrt_2 = 1.0 / std::numbers::sqrt2;
...
inline CubeFacePosition sphere_to_cube_face(const glm::dvec3 sphere)
{
    ...
    {
        const auto res = cubify(glm::dvec3(sphere.x, sphere.z, sphere.y));
        return {.face = sphere.y > 0 ? Face::Right : Face::Left, .uv = {res.y, res.x}};
    }
    if (f.x >= f.z)
    {
        const auto res = cubify(glm::dvec3(sphere.y, sphere.z, sphere.x));
        return {.face = sphere.x > 0 ? Face::Front : Face::Back, .uv = {res.y, res.x}};
    }
    const auto res = cubify(sphere);
    return {.face = sphere.z > 0 ? Top : Face::Bottom, .uv = {res.y, res.x}};
}
...
public:
    ...
    {
        glm::dvec2 linear_coords = {static_cast<double>(coords.x) / (res - 1) * 2 - 1, static_cast<double>(coords.y) / (res - 1) * 2 - 1};
        switch (face)
        {
        case Front:
            return {1.0, linear_coords.x, linear_coords.y};
        case Back:
            return {-1.0, linear_coords.x, linear_coords.y};
        case Right:
            return {linear_coords.x, 1.0, linear_coords.y};
        case Left:
            return {linear_coords.x, -1.0, linear_coords.y};
        case Top:
            return {linear_coords.x, linear_coords.y, 1.0};
        case Bottom:
            return {linear_coords.x, linear_coords.y, -1.0};
        }
        return {};
    ...
    {
        glm::dvec2 scaled = (clamp(linear_coords.uv, {-1, -1}, {1, 1}) / 2.0 + 0.5) * static_cast<double>(res - 1);
        ...
        double x = scaled.x - std::floor(scaled.x);
        double y = scaled.y - std::floor(scaled.y);
