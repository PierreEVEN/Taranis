...
float4 CalcUnshadowedAmountPCF2x2(int lightIndex, float4 vPosWorld)
{
    ...
    vShadowDepths.x = shadowMap.Sample(sampleClamp, vShadowTexCoord).r;
    vShadowDepths.y = shadowMap.Sample(sampleClamp, vShadowTexCoord + float2(vTexelUnits.x, 0.0f)).r;
    vShadowDepths.z = shadowMap.Sample(sampleClamp, vShadowTexCoord + float2(0.0f, vTexelUnits.y)).r;
    vShadowDepths.w = shadowMap.Sample(sampleClamp, vShadowTexCoord + vTexelUnits).r;

    // What weighted fraction of the 4 samples are nearer to the light than this pixel?
    float4 vShadowTests = (vShadowDepths.r >= vLightSpaceDepth) ? 1.0f : 0.0f;
